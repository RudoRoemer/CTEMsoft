...
% Footers
\lfoot{\textsf{EMsoft--2015} manual, Release 3.0-beta, \today}
\cfoot{}
...
\date{Program Manual, Release 3.0-beta, \today}
...
the library source code files.  
The name of the package has also been changed from CTEMsoft to \ctp, to reflect the fact that both SEM and TEM codes are part of the library.
In hte present beta release, only the ECP and EBSD programs are made available in Mac OS X executable form.  The source code will
be made available via GitHub at a later date this Fall.

%The 2013 version of the source code is based on the original CTEMsoft code from the late 90s and the years 
...
makefiles to use the more flexible CMake environment; to compile the \ctp\ you must have the following installed on your system: (to be added later)
