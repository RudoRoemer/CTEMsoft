...
support from an AFOSR/MURI grant FA9550-12-1-0458. [updated on 12/07/15 with corrected example .nml files]}}
...
library now has the name (on Mac OS X) \textsf{libEMSoftLib.dylib}.  As a direct consequence of this change, several of 
the subroutines can now be called from other languages.  In the present release (3.0), we make three of those routines
available to the user: \textsf{SingleEBSDPatternWrapper}, \textsf{SingleECPatternWrapper} and  \textsf{SingleKosselPatternWrapper}. Each of these routines
...
that this has only been tested on Mac OS X with the gfortran 5.2.0 compiler and IDL's \textsf{call\_external} functionality.
There are at the moment no guarantees that this will work with other configurations, but we would appreciate any feedback, error messages, and
...
\subsection{Kossel pattern calculation}
...
For EBSD and ECP pattern simulations, this interface will show both Monte Carlo and master pattern data; for the Kossel mode, only the 
