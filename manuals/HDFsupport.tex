...
write it in segments.  In HDF parlance, these segments are called ``hyperslabs''.  The $9$ routines 
...
the dimensions are consistent and will fit in the available space.  The final argument \textsf{extend} is
an optional logical argument; when present, the hyperslab will be written to an already existing dataset.
So, the first time thiese functions would be called without the \textsf{extend} argument, and for every
additional write to the dataset, the keyword must be present.

{\footnotesize
\begin{verbatim}
hdferr = HDF_writeHyperslabIntegerArray2D(dataname, wdata, hdims, offset, dim0, dim1, ..., [extend])
hdferr = HDF_writeHyperslabIntegerArray3D(dataname, wdata, hdims, offset, dim0, dim1, dim2, ..., [extend])
hdferr = HDF_writeHyperslabIntegerArray4D(dataname, wdata, hdims, offset, dim0, dim1, dim2, dim3, ..., [extend])
hdferr = HDF_writeHyperslabFloatArray2D(dataname, wdata, hdims, offset, dim0, dim1, ..., [extend])
hdferr = HDF_writeHyperslabFloatArray3D(dataname, wdata, hdims, offset, dim0, dim1, dim2, ..., [extend])
hdferr = HDF_writeHyperslabFloatArray4D(dataname, wdata, hdims, offset, dim0, dim1, dim2, dim3, ..., [extend])
hdferr = HDF_writeHyperslabDoubleArray2D(dataname, wdata, hdims, offset, dim0, dim1, ..., [extend])
hdferr = HDF_writeHyperslabDoubleArray3D(dataname, wdata, hdims, offset, dim0, dim1, dim2, ..., [extend])
hdferr = HDF_writeHyperslabDoubleArray4D(dataname, wdata, hdims, offset, dim0, dim1, dim2, dim3, ..., [extend])
\end{verbatim}
}

The hyperslabs can be read from the file using the following $9$ functions, with arguments already explained above; the
...
\caption{\label{fig:HXtalFile}Internal layout of a typical \textsf{*.xtal} structure file as represented by the Java \textsf{HDFView} program.}
\end{figure}
