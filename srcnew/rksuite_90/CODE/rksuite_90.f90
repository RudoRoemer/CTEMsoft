...
complex(kind=wp), intent(out) :: y_got, yderiv_got        !dep!
...
integer, intent(out), optional :: flag
type(rk_comm_complex_0d), intent(inout) :: comm
complex(kind=wp), intent(out) :: y_now, yderiv_now        !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
!
      htry = comm%h
      call step_c0(comm,f,comm%t,comm%y,comm%yp,comm%stages,comm%tol,htry, &
...
subroutine truerr_c0(comm,f,ier)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), dimension(:), pointer :: ge_stages                  !dep!
complex(kind=wp), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), pointer :: ge_err_estimates, y          !dep!
...
!
!  Take a step.
      call step_c0(comm,f,tsec,ge_y,ge_yp,ge_stages,ge_test1,hsec,ge_y_new, &
...
subroutine step_c0(comm,f,tnow,y,yp,stages,tol,htry,y_new,    &
...
complex(kind=wp), intent(out) ::  errest, y_new           !dep!
!!complex(kind=wp), dimension(:), intent(out) :: stages                 !dep!
complex(kind=wp), dimension(:) :: stages                 !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
subroutine stiff_c0(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), intent(in) :: v                         !dep!
...
!
complex(kind=wp), intent(in) :: alpha, beta                             !dep!
...
complex(kind=wp), intent(in) :: v1v0                                    !dep!
...
!
complex(kind=wp), intent(in) :: vec_1, vec_2              !dep!
...
!
complex(kind=wp), intent(in) :: value                                   !dep!
...
subroutine interpolate_c0(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
complex(kind=wp) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
real(kind=wp), intent(in) :: t_want                                  !indep!
real(kind=wp), intent(out) :: t_got                                  !indep!
complex(kind=wp), dimension(:), intent(out) :: y_got, yderiv_got        !dep!
integer, intent(out), optional :: flag
...
integer, intent(out), optional :: flag
type(rk_comm_complex_1d), intent(inout) :: comm
complex(kind=wp), dimension(:), intent(out) :: y_now, yderiv_now        !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
!
      htry = comm%h
      call step_c1(comm,f,comm%t,comm%y,comm%yp,comm%stages,comm%tol,htry, &
                   comm%y_new,comm%err_estimates,err,hmin,comm%phase2)
      comm%h = htry
!
!  Compare the norm of the local error to the tolerance.
...
subroutine truerr_c1(comm,f,ier)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), dimension(:,:), pointer :: ge_stages                  !dep!
complex(kind=wp), dimension(:), pointer :: ge_y, ge_yp, ge_y_new        !dep!
complex(kind=wp), dimension(:), pointer :: ge_err_estimates, y          !dep!
...
!
!  Take a step.
      call step_c1(comm,f,tsec,ge_y,ge_yp,ge_stages,ge_test1,hsec,ge_y_new, &
         ge_err_estimates,ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
subroutine step_c1(comm,f,tnow,y,yp,stages,tol,htry,y_new,    &
                     errest,err,hmin,phase_2)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
...
complex(kind=wp), dimension(:), intent(out) ::  errest, y_new           !dep!
!complex(kind=wp), dimension(:,:), intent(out) :: stages                 !dep!
complex(kind=wp), dimension(:,:) :: stages                 !dep!
!
interface
...
subroutine stiff_c1(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
!
real(kind=wp), intent(in) :: vdotv
complex(kind=wp), dimension(:), intent(in) :: v                         !dep!
...
function wt_inner_prod(vec_1,vec_2)
!
complex(kind=wp), dimension(:), intent(in) :: vec_1, vec_2              !dep!
complex(kind=wp) :: wt_inner_prod                                       !dep!
!
!
...
subroutine interpolate_c1(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
complex(kind=wp), dimension(:) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
      htry = comm%h
      call step_c2(comm,f,comm%t,comm%y,comm%yp,comm%stages,comm%tol,htry, &
                   comm%y_new,comm%err_estimates,err,hmin,comm%phase2)
      comm%h = htry
!
!  Compare the norm of the local error to the tolerance.
...
subroutine truerr_c2(comm,f,ier)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
!
!  Take a step.
      call step_c2(comm,f,tsec,ge_y,ge_yp,ge_stages,ge_test1,hsec,ge_y_new, &
         ge_err_estimates,ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
subroutine step_c2(comm,f,tnow,y,yp,stages,tol,htry,y_new,    &
                     errest,err,hmin,phase_2)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
...
subroutine stiff_c2(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
subroutine interpolate_c2(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
real(kind=wp), intent(in) :: t_want                                  !indep!
real(kind=wp), intent(out) :: t_got                                  !indep!
real(kind=wp), intent(out) :: y_got, yderiv_got        !dep!
integer, intent(out), optional :: flag
...
integer, intent(out), optional :: flag
type(rk_comm_real_0d), intent(inout) :: comm
real(kind=wp), intent(out) :: y_now, yderiv_now        !dep!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
      real(kind=wp), intent(in) :: t                                 !indep!
...
!
      htry = comm%h
      call step_r0(comm,f,comm%t,comm%y,comm%yp,comm%stages,comm%tol,htry, &
                   comm%y_new,comm%err_estimates,err,hmin,comm%phase2)
      comm%h = htry
!
!  Compare the norm of the local error to the tolerance.
...
subroutine truerr_r0(comm,f,ier)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
real(kind=wp), dimension(:), pointer :: ge_stages                  !dep!
real(kind=wp), pointer :: ge_y, ge_yp, ge_y_new        !dep!
real(kind=wp), pointer :: ge_err_estimates, y          !dep!
real(kind=wp), pointer :: ge_assess, weights           !shp-dep!
!
ge_stages => comm%ge_stages
ge_y => comm%ge_y
...
!
!  Take a step.
      call step_r0(comm,f,tsec,ge_y,ge_yp,ge_stages,ge_test1,hsec,ge_y_new, &
         ge_err_estimates,ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
subroutine step_r0(comm,f,tnow,y,yp,stages,tol,htry,y_new,    &
                     errest,err,hmin,phase_2)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
...
real(kind=wp), intent(out) ::  errest, y_new           !dep!
!real(kind=wp), dimension(:), intent(out) :: stages                 !dep!
real(kind=wp), dimension(:) :: stages                 !dep!
!
interface
   function f(t,y)
      use rksuite_90_prec, only:wp
...
subroutine stiff_r0(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
function wt_inner_prod(vec_1,vec_2)
!
real(kind=wp), intent(in) :: vec_1, vec_2              !dep!
...
subroutine interpolate_r0(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
real(kind=wp), dimension(:) :: ytemp, xstage            !dep!
!
interface 
   function f(t,y)
      use rksuite_90_prec, only:wp
...
!
      htry = comm%h
      call step_r2(comm,f,comm%t,comm%y,comm%yp,comm%stages,comm%tol,htry, &
                   comm%y_new,comm%err_estimates,err,hmin,comm%phase2)
      comm%h = htry
!
!  Compare the norm of the local error to the tolerance.
...
subroutine truerr_r2(comm,f,ier)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
!
!  Take a step.
      call step_r2(comm,f,tsec,ge_y,ge_yp,ge_stages,ge_test1,hsec,ge_y_new, &
         ge_err_estimates,ge_err)
!
!  The primary integration is using a step size of H_OLD and the
!  secondary integration is using the smaller step size 
...
subroutine step_r2(comm,f,tnow,y,yp,stages,tol,htry,y_new,    &
                     errest,err,hmin,phase_2)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
...
subroutine stiff_r2(comm,f,toomch,sure_stiff)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
...
subroutine interpolate_r2(comm,f,t_want,y_want,yderiv_want)
!
! Part of rksuite_90 v1.0 (Aug 1994)
!         software for initial value problems in ODEs
!
