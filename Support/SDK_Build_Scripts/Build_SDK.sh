...
#-------------------------------------------------
# Unpack CMake
sudo tar -xvzf ${SDK_INSTALL}/cmake-3.4.1-${HOST_SYSTEM}-x86_64.tar.gz

#-------------------------------------------------
# Get CMake on our path
if [[ "$HOST_SYSTEM" = "Darwin" ]];
then
  export PATH=$PATH:${SDK_INSTALL}/cmake-3.4.1-${HOST_SYSTEM}-x86_64/CMake.app/Contents/bin
else
  export PATH=$PATH:${SDK_INSTALL}/cmake-3.4.1-${HOST_SYSTEM}-x86_64/bin
fi
...
#-------------------------------------------------
# Build HDF5 Library
sudo ./Build_HDF5.sh "${SDK_INSTALL}" ${PARALLEL_BUILD}
...
then
  ...
  then
    ...
    sudo $DOWNLOAD_PROG  "http://ftp.stack.nl/pub/users/dimitri/Doxygen-1.8.11.dmg" -o "${EMsoft_SDK}/Doxygen-1.8.11.dmg"
