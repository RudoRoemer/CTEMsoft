...
then
  echo "-------------------------------------------"
  echo " Downloading fortrancl Version ${CLFORTRAN_VERSION}"
  echo "-------------------------------------------"
...
mkdir Build
cd Build
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=$SDK_PARENT/$CLFORTRAN_INSTALL_NAME-Debug ../
...
mkdir zRel
cd zRel
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$SDK_PARENT/$CLFORTRAN_INSTALL_NAME-Release ../
make -j$PARALLEL_BUILD
make install
