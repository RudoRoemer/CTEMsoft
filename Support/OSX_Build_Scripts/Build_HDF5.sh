...
mkdir Build
cd Build
cmake  -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Debug -DHDF5_BUILD_TOOLS=ON -DHDF5_BUILD_FORTRAN=ON -DHDF5_BUILD_WITH_INSTALL_NAME=ON -DHDF5_BUILD_CPP_LIB=ON -DHDF5_BUILD_HL_LIB=ON -DCMAKE_INSTALL_PREFIX=$SDK_INSTALL/hdf5-${version}-Debug ../
make -j${PARALLEL_BUILD}
make install
cd ../
mkdir zRel
cd zRel
cmake  -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DHDF5_BUILD_TOOLS=ON -DHDF5_BUILD_FORTRAN=ON -DHDF5_BUILD_WITH_INSTALL_NAME=ON -DHDF5_BUILD_CPP_LIB=ON -DHDF5_BUILD_HL_LIB=ON -DCMAKE_INSTALL_PREFIX=$SDK_INSTALL/hdf5-${version}-Release ../
make -j${PARALLEL_BUILD}
make install
...
echo "set(HDF5_INSTALL \"\${EMSoft_SDK_ROOT}/hdf5-${version}-\${BUILD_TYPE}\")" >> "$SDK_INSTALL/EMSoft_SDK.cmake"
echo "set(HDF5_DIR \"\${EMSoft_SDK_ROOT}/hdf5-${version}-\${BUILD_TYPE}/share/cmake\")" >> "$SDK_INSTALL/EMSoft_SDK.cmake"
echo "" >> "$SDK_INSTALL/EMSoft_SDK.cmake"
